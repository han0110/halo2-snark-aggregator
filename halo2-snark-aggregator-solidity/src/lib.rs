...
use halo2_snark_aggregator_circuit::verify_circuit::{
    load_instances, load_params, load_transcript, Halo2VerifierCircuit,
};
...
pub fn verify_circuit_sol_generator<C: CurveAffine, E: MultiMillerLoop<G1Affine = C>>(
    mut output_folder: std::path::PathBuf,
    template_folder: std::path::PathBuf,
    _nproofs: usize,
) {
    let verify_circuit_params = load_params::<C>(&mut output_folder, "verify_circuit.params");
    let verify_circuit_instance =
        load_instances::<E>(&mut output_folder, "verify_circuit_instance.data");
    let verify_circuit_transcript =
        load_transcript::<C>(&mut output_folder, "verify_circuit_proof.data");
    let verify_circuit_vk = {
        output_folder.push("verify_circuit.vkey");
        let mut fd = std::fs::File::open(output_folder.as_path()).unwrap();
        output_folder.pop();
    ...
    let mut transcript = PoseidonTranscriptRead::<_, C, _, PoseidonEncode<_>, 9usize, 8usize>::new(
        &verify_circuit_transcript[..],
        ctx,
        schip,
        8usize,
        33usize,
...

    {
        ...
        output_folder.push("verifier.sol");
        let mut fd = std::fs::File::create(output_folder.as_path()).unwrap();
        output_folder.pop();
        write!(&mut fd, "{}", template).unwrap();
    }

    print!("generate solidity succeeds");
}
